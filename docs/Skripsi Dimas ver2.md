...
Penelitian ini berhasil membangun fitur penjadwalan otomatis pada Sistem Informasi Operasional Laboratorium Komputer (SIOPAL) menggunakan teknik _Query Filtering Eloquent_ berlapis dengan metode pengembangan _Rapid Application Development_ (RAD). Penerapan metode RAD melalui lima tahapan — _Business Modelling_, _Data Modelling_, _Process Modelling_, _Application Generation_, dan _Testing and Turnover_ — memungkinkan keterlibatan langsung administrator laboratorium pada setiap tahapan, sehingga fitur yang dihasilkan sesuai dengan kebutuhan operasional penjadwalan di Laboratorium Komputer Fakultas Ilmu Komputer Universitas Dian Nuswantoro.

Arsitektur data yang dirancang mampu merepresentasikan empat batasan utama penjadwalan, yaitu kapasitas laboratorium, ketersediaan _software_, jam operasional, dan ketersediaan waktu. Algoritma _filtering_ diimplementasikan dalam dua lapisan — lapisan basis data dan lapisan aplikasi — melalui enam tahap penyaringan berurutan. Sistem menyediakan dua mode input, yaitu input satuan melalui antarmuka _wizard_ dan _import_ massal melalui berkas Excel. Kedua mode berhasil menghasilkan jadwal yang bebas konflik.

Keberhasilan sistem ditunjukkan melalui hasil pengujian _Black Box Testing_ terhadap 30 skenario uji yang seluruhnya menghasilkan status **Valid** dengan tingkat keberhasilan **100%**. Hasil _User Acceptance Testing_ (UAT) terhadap 35 responden menunjukkan tanggapan positif dengan rincian aspek Fungsionalitas **92,9%**, Kemudahan Penggunaan **92,9%**, Keandalan **92,4%**, Efisiensi **93,0%**, serta Kesesuaian Kebutuhan dan Kepuasan **92,7%**, sehingga diperoleh rata-rata penerimaan keseluruhan sebesar **92,8%** yang termasuk dalam kategori **Sangat Layak**. Hasil ini membuktikan bahwa fitur penjadwalan otomatis SIOPAL tidak hanya valid secara teknis, tetapi juga diterima dengan baik oleh pengguna dan layak untuk diimplementasikan secara operasional.
