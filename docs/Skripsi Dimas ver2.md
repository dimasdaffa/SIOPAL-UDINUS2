...
Proses penjadwalan praktikum di Laboratorium Komputer Fakultas Ilmu Komputer Universitas Dian Nuswantoro masih dilakukan secara manual menggunakan _spreadsheet_, sehingga rentan terhadap _human error_ berupa bentrokan jadwal, ketidaksesuaian kebutuhan _software_, dan inefisiensi waktu. Permasalahan ini tergolong sebagai _Constraint Satisfaction Problem_ (CSP) yang melibatkan empat _constraints_ utama: ketersediaan _software_, kapasitas laboratorium, jam operasional, dan ketersediaan waktu. Penelitian ini bertujuan membangun fitur penjadwalan otomatis pada Sistem Informasi Operasional Laboratorium (SIOPAL) menggunakan teknik _Query Filtering Eloquent_ berlapis pada _framework_ Laravel 12 dan Filament 3. Metode pengembangan yang digunakan adalah _Rapid Application Development_ (RAD). Algoritma _filtering_ diimplementasikan dalam dua lapisan: lapisan basis data menggunakan Eloquent `where()` dan `whereHas()`, serta lapisan aplikasi menggunakan Collection `filter()`. Sistem menyediakan dua mode input, yaitu _wizard_ satuan dan _import_ massal via Excel. Hasil pengujian _Black Box Testing_ terhadap 30 skenario menunjukkan tingkat keberhasilan 100%, sementara _User Acceptance Testing_ (UAT) terhadap 35 responden menghasilkan tingkat penerimaan sebesar 92,8% dengan kategori Sangat Layak. Dengan demikian, fitur penjadwalan otomatis SIOPAL terbukti valid dan layak diimplementasikan secara operasional.

**Kata Kunci**: _Constraint Satisfaction Problem_, _Query Filtering Eloquent_, Penjadwalan Otomatis, Laravel, _Rapid Application Development_, SIOPAL
...
Penelitian ini berhasil membangun fitur penjadwalan otomatis pada Sistem Informasi Operasional Laboratorium Komputer (SIOPAL) menggunakan teknik _Query Filtering Eloquent_ berlapis dengan metode pengembangan _Rapid Application Development_ (RAD) yang terdiri dari tahapan _Business Modelling_, _Data Modelling_, _Process Modelling_, _Application Generation_, dan _Testing and Turnover_. Penerapan metode RAD memungkinkan keterlibatan langsung administrator laboratorium sebagai pengambil keputusan pada setiap tahapan, sehingga fitur yang dihasilkan sesuai dengan kebutuhan operasional penjadwalan di Laboratorium Komputer Fakultas Ilmu Komputer Universitas Dian Nuswantoro.

Arsitektur data dirancang dengan tujuh entitas utama dan tiga tabel pivot yang mampu merepresentasikan empat _constraints_ utama penjadwalan laboratorium, yaitu _constraint_ kapasitas melalui atribut `pc_siap`, _constraint_ ketersediaan _software_ melalui tabel pivot `course_software` dan `lab_software`, _constraint_ jam operasional melalui atribut `operating_start` dan `operating_end`, serta _constraint_ ketersediaan waktu melalui relasi antara tabel `schedules`, `time_slots`, dan `laboratoria`. Rancangan ini juga mengakomodasi _constraints_ tambahan berupa _break times_, sesi waktu, dan prioritas lab-prodi.

Algoritma _filtering_ diimplementasikan melalui enam tahap berurutan yang terbagi dalam dua lapisan: lapisan basis data menggunakan Eloquent `where()` dan `whereHas()` untuk filter kapasitas dan _software_, serta lapisan aplikasi menggunakan Laravel Collection `filter()` untuk filter konflik jadwal, sesi waktu, _break times_, dan pengurutan prioritas. Sistem menyediakan dua mode input, yaitu input satuan melalui antarmuka _wizard_ dan _import_ massal melalui berkas Excel yang menggunakan teknik pengurutan SKS serta pelacakan slot _in-memory_ untuk optimasi performa. Kedua mode berhasil menghasilkan jadwal yang bebas konflik (_clash-free_).

Keberhasilan sistem ditunjukkan melalui hasil pengujian _Black Box Testing_ terhadap 30 skenario uji yang seluruhnya menghasilkan status **Valid** dengan tingkat keberhasilan **100%**. Hasil _User Acceptance Testing_ (UAT) terhadap 35 responden menunjukkan tanggapan positif terhadap kelima aspek penilaian, dengan rincian aspek Fungsionalitas memperoleh **92,9%**, Kemudahan Penggunaan **92,9%**, Keandalan **92,4%**, Efisiensi **93,0%**, serta Kesesuaian Kebutuhan dan Kepuasan **92,7%**. Dari kelima aspek tersebut, diperoleh rata-rata persentase penerimaan keseluruhan sebesar **92,8%** yang termasuk dalam kategori **Sangat Layak**.

Hasil ini membuktikan bahwa penerapan teknik _Query Filtering Eloquent_ berlapis efektif untuk menyelesaikan _Constraint Satisfaction Problem_ dalam domain penjadwalan laboratorium. Fitur penjadwalan otomatis SIOPAL tidak hanya berhasil mengatasi tantangan teknis berupa validasi empat _constraints_ utama secara otomatis, tetapi juga memberikan manfaat nyata dalam meningkatkan efisiensi proses penyusunan jadwal praktikum. Tingkat penerimaan pengguna yang tinggi membuktikan bahwa solusi ini telah memenuhi ekspektasi pengguna akhir dalam hal fungsionalitas, kemudahan penggunaan, keandalan, efisiensi, dan kesesuaian kebutuhan, sehingga layak untuk diimplementasikan secara operasional di Laboratorium Komputer Fakultas Ilmu Komputer Universitas Dian Nuswantoro.
